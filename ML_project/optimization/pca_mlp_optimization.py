...
trial 0 so TPE starts from a non-random baseline. No pruning — every
trial runs all k folds to completion. Companion script
pca_mlp_optimization_pruning.py is the same search with a fold-level
MedianPruner bolted on; run both to compare throughput vs. search quality.
...
def evaluate_config(cfg: dict):
    """
    Run k-fold CV for one config. Every fold runs to completion —
    no intermediate reporting, no pruning.
    """
...
def run_trial(cfg: dict) -> dict:
    ...
    print(f"\n>>> Trial: {name}")
    _, summary, score = evaluate_config(cfg)
    record = {
        "name": name,
        "config": dict(cfg),
        "score": score,
...
#### OPTUNA JOINT SEARCH ####
print("\n" + "=" * 72 + f"\nOptuna TPE — {TRIALS} trials (no pruning)\n" + "=" * 72)
...
if not all_trials:
    raise RuntimeError("No completed trials — every trial failed.")
